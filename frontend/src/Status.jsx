            ...
            <Carousel showThumbs={false} dynamicHeight={true}>
              {findStatus()?.statusContent?.map((content, index) => {
                return <div key={index}>
                  ...
                  <div className="user-image" style={{ zIndex: '222', position: 'absolute', top: '14px', left: '25px' }}>
                    <img style={{ width: '40px', height: '40px' }} src={findStatus()?.photo} alt="" />
                  </div>
                  <p style={{ position: 'absolute', top: '35px', left: '75px', color: 'white', fontWeight: 'bold', fontSize: '0.85rem' }}>{content.statusUpdatedAt?.formattedTime}</p>
                  <div style={{ position: 'absolute', top: '15px', left: '75px', color: 'white' }} className="user-name">
